...
def get_lr_scheduler(optimizer, schedule_mode='exp_lin', warm_up_len=5, ramp_down_start=15, ramp_down_len=25,
                     last_lr_value=0.04, nr_of_epochs=50):
    if schedule_mode in {"exp_lin", "exp_down", "cos_cyc", "mul_lin_down"}:
        return torch.optim.lr_scheduler.LambdaLR(optimizer,
                                                 get_scheduler_lambda(
                                                     ...
                                                     ramp_down_len=ramp_down_len,
                                                     last_lr_value=last_lr_value,
...
class M(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        results = {"loss": loss, "l1_loss": l1_reg}
    ...
    def training_epoch_end(self, outputs):
        ...
        da_loss = torch.stack([x['da_loss'] for x in outputs]).mean()
        gp_da_loss = torch.stack([x['gp_da_loss'] for x in outputs]).mean()

        logs = {'train.loss': avg_loss,'train.l1_loss': l1_loss, "train.da_loss": da_loss,
                "train.gp_da_loss":gp_da_loss, 'step': self.current_epoch}
