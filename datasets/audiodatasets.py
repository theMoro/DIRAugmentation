...
class ObjectCacher:
    def __init__(self, get_obj_func, dataset_name, obj_name="",
                 cache_path="/home/tobi/dataset/dcase22/cached_dataset/", verbose=True):
...
class PreprocessDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(data) == 2:
            return self.preprocessor(self.dataset[index][0]), self.dataset[index][1]
...
class FilesCachedDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            returned_data = torch.load(cpath)
            x, file, label = returned_data[:3]
            return x, file, label
...
class SimpleSelectionDatasetWithIndex(Dataset):
    """A dataset that selects a subsample from a dataset based on a set of sample ids.
    ...
    def __getitem__(self, index):
        return self.dataset[self.available_indexes[index]], index
