...
class Network(nn.Module):
    def __init__(self, config,ff_weight_anticolapse_limit=0.5):
        ...
        if channels_multiplier != 2:
            print(f"channels_multiplier={channels_multiplier}")
        ...
        if config.get("attention_avg"):
            ...
            no_bn_out = config.get("no_bn_out") or False  # haha to remove
            ff_list += [nn.Conv2d(
                n_channels[2],
                n_classes,
                kernel_size=1,
...
def get_model(rho=7, in_channels=1, arch="cp_resnet", cut_stage2=0, cut_stage3=0, config_only=False, n_classes=10,
