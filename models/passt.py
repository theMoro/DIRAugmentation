...
from models.helpers.vit_helpers import update_default_cfg_and_kwargs, DropPath, trunc_normal_, \
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        # self.conv = nn.Conv2d(12, 12, 3, padding=1)
    ...
    def forward(self, x):
        ...
        # conv part
        # x = x.reshape((B, N, 12, 64)).permute(0, 2, 1, 3)
        # conv_output = self.conv(x)
        # conv_output = conv_output.permute(0, 2, 1, 3).reshape((B, N, C))
...
class PaSST(nn.Module):
    ...
    def __init__(self, u_patchout=0, s_patchout_t=0, s_patchout_f=0, img_size=(128, 998), patch_size=16, stride=16,
                 ...
                 act_layer=None, weight_init='', one_sec_embeds=False):
        """
        Args:
        ...
        self.one_sec_embeds = one_sec_embeds
    ...
    def forward(self, x):
        global first_RUN
        if first_RUN: print("x", x.size())  # 12 x 1 x 128 x 1000

        if self.one_sec_embeds:
            x = rearrange(x, 'b c f (slices t) -> (slices b) c f t', slices=10)  # 120 x 1 x 128 x 100
            if first_RUN: print("x rearanged", x.size())
        ...
        if self.head_dist is not None:
            ...
            if self.one_sec_embeds:
                features = rearrange(features, '(slices b) y -> slices b y', slices=10)
                if first_RUN: print("forward_features rearranged", features.size())
                features = torch.mean(features, dim=0)
                if first_RUN: print("forward_features after mean", features.size())
...
def get_model(arch="passt_s_swa_p16_128_ap476", pretrained=True, n_classes=527, in_channels=1, fstride=10,
              tstride=10,
              input_fdim=128, input_tdim=998, u_patchout=0, s_patchout_t=0, s_patchout_f=0, one_sec_embeds=False
              ):
    """
    ...
    model = model_func(pretrained=pretrained, num_classes=n_classes, in_chans=in_channels,
                       ...
                       s_patchout_t=s_patchout_t, s_patchout_f=s_patchout_f, one_sec_embeds=one_sec_embeds)
